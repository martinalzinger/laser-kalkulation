...
- **Schnittgeschwindigkeit (`SPEED`, `effCutSpeed`):** Stahl 1–5 mm = echte TruTops-LTT-Werte (Konturart
  ...
  Konturen) konkav zwischen Klein (≤120 mm) und Gross (≥600 mm); die Konturgröße wird zusätzlich auf die
  **Teilgröße** (`partMaxDim`) gedeckelt (kurze Geraden → nie Topspeed). Nur bei Gross-Gruppen (bis `maxT`).
